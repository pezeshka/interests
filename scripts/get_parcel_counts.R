# This script pulls parcel count data for all municipalities in Massachusetts. 
...
get_parcel_counts <- function(year) {
  # Print a message to the console to show progress
  message(paste("Fetching parcel counts for year:", year))
  ...
  resp <- POST(
    ...
    query = list(
      ...
      rdExportTableID = "xtParcels",
      rdExportFilename = paste0("ParcelCountsByUseCode", year),
  ...
  if (status_code(resp) != 200) {
    warning(paste("Failed to download counts for year", year, "- Status:", status_code(resp)))
    return(NULL)
  }
...
parcel_df <- purrr::map(years_to_fetch, get_parcel_counts) |> 
  bind_rows()

readr::write_csv(parcel_df, file = paste0(write_path, "parcel_counts.csv"))